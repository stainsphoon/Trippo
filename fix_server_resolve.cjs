...
const originalRoute = code.substring(
  code.indexOf(`app.post('/app-api/destinations/resolve', async (req, res) => {`),
  code.indexOf(`  app.get('/app-api/destinations/seed',`)
);
...
const newRoute = `app.post('/app-api/destinations/resolve', async (req, res) => {
    try {
      ...
      try {
        ...
        // googlePlaceIdMappingStore.set(externalId, finalizedDestination.id); // Also done via upsertRuntimeDestination potentially, but let's be explicit
